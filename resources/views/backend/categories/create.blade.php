...
@section('title', "Create Category | Bloom & Glow Mbita")
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6" data-aos="fade-up">
            <div>
                ...
                <p class="text-sm sm:text-base text-gray-600 font-['Plus_Jakarta_Sans'] mt-2">
                    Create a new service category to organize your offerings and availability.
                </p>
            </div>
...
<section class="py-8 sm:py-12 bg-gray-50/50 min-h-[500px]">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden p-6 sm:p-8" x-data="{ hasImage: {{ old('has_image') ? 'true' : 'false' }}, fileName: '' }">
            
            <form action="{{ route('management.categories.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <select id="available" 
                        ...
                        <option value="yes" {{ old('available') === 'yes' ? 'selected' : '' }}>Available</option>
                        <option value="no" {{ old('available') === 'no' ? 'selected' : '' }}>Not Available</option>
                    </select>
                    @error('available')
                        <p class="text-xs text-red-600 mt-1.5">{{ $message }}</p>
                    @enderror
                ...
                <div x-show="hasImage" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 transform -translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0" style="display: none;">
                    <label class="block text-xs font-semibold uppercase tracking-wider text-gray-700 font-['Outfit'] mb-2">Upload Image <span class="text-red-500">*</span></label>
